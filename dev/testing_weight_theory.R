...
# Potential provision
pot_weights
# es_potential_base = hab1 * pot_weights:
es_potential_base = hab1 * pot_weights
# In year 2 expect water == 1, 0.45, 0.22, soil == 0.4, 0.54, 1.1, etc.:
es_potential_base * hab2
# And should change again in y3:
es_potential_base * hab3

# Potential wellbeing:
# This is calc as hab * wb_base,
#   where wb_base = hab1 * es_potential_base,
wb_base <- hab1 * es_potential_base
wb_base * hab1
wb_base * hab2
wb_base * hab3

# Would they diverge?
all.equal(es_potential_base* hab3, wb_base * hab3)
# They would not?
...
custom_weights
# es_potential_base = hab1 * pot_weights:
es_potential_base2 = hab1 * custom_weights
# In year 2 expect water == 1, 0.45, 0.22, soil == 0.4, 0.54, 1.1, etc.:
es_potential_base2 * hab2
# And should change again in y3:
es_potential_base2 * hab3
...
all.equal(es_potential_base2* hab3, wb_base2 * hab3)
# Not eaual, so this is about the fact that the es_potential_base isn't 1, because of the
