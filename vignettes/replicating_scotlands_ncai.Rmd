...
openNCAI includes two helper functions - `create_ncai_template()` and `read_ncai_template` - to help assemble data and get it into the right format for calculation. 
...
ncai_options <- data.frame(
  ...
  `Description` = c(
    ...
    "Unaggregated annual matrices representing potential wellbeing values per habitat and ecosystem service.",
    "Unaggregated annual matrices representing the likely flow of ecosystem services per habitat and ecosystem service.",
...
Having access to intermediate stages of the calculation means we can, for example, look at how the changing wellbeing contribution (**extent** of habitats weighted by potential per-unit provision of services and by importance to the population) and the changing estimated flow of ecosystem services (based on **condition** of habitats) and how these relate to the overall valuation.
...
driver_plot <- ggplot(plot_data_long, aes(x = Year, y = Value, color = Index, shape = Index)) +
  ...
  annotate("label", x = Inf, y = -Inf,
           label = paste0("Correlations:\nOverall/Wellbeing: ", cor_wellbeing,
