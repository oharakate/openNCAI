test_that("calc_potential_weights handles missing divisors", {
  ...
  # FIX: Align names even for error tests to ensure we hit the
  # divisor check rather than the name-mismatch check
  rownames(df) <- c("h1", "h2")
  colnames(df) <- c("es1", "es2")

  expect_error(
    openNCAI:::calc_potential_weights(df, divisor = NULL, custom_divisor_matrix = NULL,
...
test_that("calc_potential_weights catches dimension mismatches", {
  ...
  # FIX: Align names for df.
  # (Note: wrong_dim has 3 rows, so it will still trigger the dimension error)
  rownames(df) <- c("h1", "h2")
  colnames(df) <- "es1"
