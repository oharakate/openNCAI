test_that("esppu_scores_to_weights handles missing divisors", {
  ...
  expect_error(
    esppu_scores_to_weights(df, divisor = NULL, custom_divisor_matrix = NULL, h_tree, es_tree),
    "You must provide either a 'divisor' or a 'custom_divisor_matrix'"
  )
})

test_that("esppu_scores_to_weights works with a single divisor", {
  df <- data.frame(a = c(10, 20), b = c(30, 40))
  h_tree <- list(cat = c("h1", "h2"))
  es_tree <- list(cat = c("es1", "es2"))

  # Pass trees here
  res <- esppu_scores_to_weights(df,
                                 divisor = 10,
...
test_that("esppu_scores_to_weights works with custom matrix", {
...
test_that("esppu_scores_to_weights catches dimension mismatches", {
  df <- data.frame(a = c(10, 20))
