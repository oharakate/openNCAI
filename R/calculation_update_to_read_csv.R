...
wb <- read_csv("dev/wellbeing_base.csv", col_names = FALSE)
espb <- read_csv("dev/ecosystem_potential_base.csv", col_names = FALSE)
# Ecosystem service providing potential per SPU matrix:
esppu <- read_csv("dev/ecosystem_potential_per_service_provisioning_unit.csv", col_names = FALSE)
# Extent data all years (Scotland)
eds_w_totals <- read_csv("dev/ecosystem_area.csv", col_names = TRUE)
# R/m totals
eds <- eds_w_totals[-nrow(eds_w_totals),]
# Indicator directory
scot_indd <- read_csv("dev/scot_indicator_directory.csv", col_names = TRUE)
# Condition Indicator Relevance Matrix 1 (#2 Pollution orthophosphate etc.)
scot_cirm1 <- read_csv("dev/scot_cirm1.csv", col_names = FALSE)

espb <- as.data.frame(espb)
esppu <- as.data.frame(esppu)
wb <- as.data.frame(wb)
eds_w_totals <- as.data.frame(eds_w_totals)
scot_indd <- as.data.frame(scot_indd)
scot_cirm1 <- as.data.frame(scot_cirm1)

# ES Potential ('Scotland weights')
# Chris has typed these manually below, but here they are as csv:
eswr_scot_sections <- read_csv("dev/scotland_weight_raw_service_sections.csv", col_names = FALSE)
eswr_prov <- read_csv("dev/scotland_weights_raw_provisioning.csv", col_names = FALSE)
eswr_regu <- read_csv("dev/scotland_weights_raw_regulationmaintenance.csv", col_names = FALSE)
eswr_cult <- read_csv("dev/scotland_weights_raw_cultural.csv", col_names = FALSE)
eswr_scot_sections <- as.data.frame(eswr_scot_sections)
eswr_prov <- as.data.frame(eswr_prov)
eswr_regu <- as.data.frame(eswr_regu)
eswr_cult <- as.data.frame(eswr_cult)
...
imp_rtw_within <- function(scores, between_weights, index) {
  ...
  # within weights?
  # The index argument of read.csv is replaced by col_names
...
scot_ciwm1 <- read_csv("dev/orig_scot_cirm1_weights.csv", col_names = FALSE)
...
cirm1 <- read_csv("dev/scot_cirm1.csv", col_names = FALSE)
names(cirm1) <- all_service_labels
cirm1[is.na(cirm1)] <- 0

cirm2 <- read_csv("dev/scot_cirm2.csv", col_names = FALSE)
names(cirm2) <- all_service_labels
cirm2[is.na(cirm2)] <- 0

cirm3 <- read_csv("dev/scot_cirm3.csv", col_names = FALSE)
...
scot_ciwm2 <- read_csv("dev/scot_ciwm2.csv", col_names = FALSE)
scot_ciwm2[is.na(scot_ciwm2)] <- 0
names(scot_ciwm2) <- all_service_labels
scot_ciwm3 <- read_csv("dev/scot_ciwm3.csv", col_names = FALSE)
...
# cis <- read_csv("dev/scot_ci1_stbi.csv", col_names = FALSE)
# names(cis) <- c("raw1")
...
stbi1 <- read_csv("dev/scot_ci1_stbi.csv", col_names = FALSE)
stbi2 <- read_csv("dev/scot_ci2_stbi.csv", col_names = FALSE)
stbi3 <- read_csv("dev/scot_ci3_stbi.csv", col_names = FALSE)
...
cis_full_scot <- read_csv("dev/scot_year_ci_matrix.csv", col_names = TRUE)
...
ciwm_to_cirm <- function(filepath, #path to cirm
  ...
  df <- read_csv(filepath, col_names = has_header)
